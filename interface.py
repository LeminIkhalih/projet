...
if not os.path.isfile("nom_utilisateur.txt"):
    with open ("nom_utilisateur.txt", 'wb') as file:
        pass

else:
    with open("nom_utilisateur.txt", 'r') as file:
        for line in file:
...
# Send Message
    ...
    def change_username_main(self, username, default=False):
        ...
        def write_usernames():
            with open('nom_utilisateur.txt', 'w') as filer:
...
# promps user to Clear username history (deletes nom_utilisateur.txt file and clears saved_username list)
...
# opens window showing username history (possible temp feature)
    def view_username_history(self):
        with open("nom_utilisateur.txt", 'r') as usernames:
